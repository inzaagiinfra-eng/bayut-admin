...
const RejectedProperties = () => {
    ...
    const getallproperties = async () => {
        ...
        const response = await axios.get(
            "http://localhost:8000/api/properties?status=rejected",
                    ...
                    <tbody>
                        {allproperties.map((item, index) => (
                            <tr key={item._id}>
                                ...
                                <td className="border border-gray-300 px-4 py-2">
                                    <Link
                                        className="bg-yellow-500 hover:bg-yellow-400 px-2 flex gap-1 justify-center items-center py-1 cursor-pointer rounded-2xl text-white text-sm"
